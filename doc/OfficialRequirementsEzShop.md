# Official Requirements Document - EZ Shop 

Authors: Maurizio Morisio 
...
has two or few more cash registers.
...
| Shop owner | Owns the shop, monitors its profitability and customer satisfaction|
...
| FR 5 | Manage customers and loyalty cards |
| FR5.1 | Define or modify a customer |
| FR5.2 | Delete a customer  |
| FR5.3 | Search a customer  |
| FR5.4 | List  all customers  |
| FR5.5 | Create a loyalty card  |
| FR5.6 | Attach loyalty card to a customer  |
| FR5.7 | Modify points on a loyalty card   |
...
| Function | Administrator | Shop Manager | Cashier | CreditCardCircuit | Supplier |
| -------- | ----- | ------------ | ------- | ------ | ------ |
| FR1 | yes | no | no | no| no |
| FR3| yes  | yes |  no | no | no |
| FR3.3| yes  | yes |  yes | no | no |
| FR4 | yes | yes | no | no | no |
| FR5 | yes  | yes|  yes| no | no |
|FR6 | yes  | yes |  yes| no | no|  
|FR7 | yes | yes| yes | no| no|
...
| ------------- |:-------------:| :-----| -----:|
|  NFR1     | Usability | All functions should be used with no specific training | All FR |
...
# System Design
System design shows the components (hardware and software) that are included in EzShop. 
EZShop is designed following a client server model. Clients are built around cash registers, each having cash drawer,
 bar code reader, printer for sale tickets, credit card and loyalty card reader, computer. Clients are connected via internet link to a server. The server
 has a back end application storing data (catalogue of product types, inventory, transactions, orders etc).
 All hardware components are acquired from the market, software components are developed. 
...
class EZShopFrontEndApplication
class EZShopBackEndApplication

EZShop o-- EZShopServer
EZShop o-- "*" EZShopClient

EZShopClient o-- BarCodeReader
EZShopClient o-- CardReader
EZShopClient o-- Printer
EZShopClient o-- LoyaltyCardReader
EZShopClient o-- CashDrawer
EZShopClient o-- EZShopFrontEndApplication
EZShopClient o-- ClientComputer

EZShopServer o-- EZShopBackEndApplication
EZShopServer o-- ServerComputer
```
...
node ClientComputer
node ServerComputer
node CreditCardGateway
artifact EZShopFrontEndApplication
artifact EZShopBackEndApplication

ClientComputer "*" -- ServerComputer
ServerComputer -- CreditCardGateway

EZShopFrontEndApplication -> ClientComputer
