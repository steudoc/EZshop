...
# ---------------------------
# REIMBURSE RETURN TESTS 
# ---------------------------

def test_reimburse_return_invalid_id(client, auth_tokens):

    for role in ["admin", "manager"]:
        #Reimburse return
        reimburse_resp = client.patch(
            BASE_URL + f"/returns/-2/reimburse",
            headers=auth_header(auth_tokens, role)
        )

        # Assert response
        assert reimburse_resp.status_code == 400

def test_reimburse_return_unauthenticated(client, auth_tokens,closed_return_creation):
    return_id = closed_return_creation["id"]

    #Close return
    reimburse_resp = client.patch(
        BASE_URL + f"/returns/{return_id}/reimburse",
        headers=auth_header(auth_tokens, "unauthorized")
    )
    # Assert response
    assert reimburse_resp.status_code == 401
    
def test_reimburse_return_not_found(client, auth_tokens):
    for role in ["admin", "manager"]:
        #Reimburse return
        reimburse_resp = client.patch(
            BASE_URL + f"/returns/111/reimburse",
            headers=auth_header(auth_tokens, role)
        )        
        # Assert response
        assert reimburse_resp.status_code == 404

def test_reimburse_return_invalid_state(client, auth_tokens, return_creation):
    ...
    for role in ["admin", "manager"]:
        ...
        # Assert response
        assert reimburse_resp.status_code == 420

def test_reimburse_return_success_admin(client, auth_tokens, closed_return_creation):
    return_id = closed_return_creation["id"]
    ...
    initial_balance = initial_balance.json()["balance"]
    #Reimburse return
    reimburse_resp = client.patch(
        BASE_URL + f"/returns/{return_id}/reimburse",
        headers=auth_header(auth_tokens, "admin")
    ...
    assert reimburse_resp.json()["refund_amount"] == 2.99
    ...
    final_balance = final_balance.json()["balance"]

    assert final_balance == initial_balance-2.99

def test_reimburse_return_success_manager(client, auth_tokens, closed_return_creation):
    return_id = closed_return_creation["id"]
...
def test_reimburse_return_success_cashier(client, auth_tokens, closed_return_creation):
    return_id = closed_return_creation["id"]
    ...
    assert reimburse_resp.status_code == 403
