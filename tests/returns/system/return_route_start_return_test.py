...
@pytest.fixture
def paid_sale_creation(client, auth_tokens):
...
def test_start_return_success_authorized_users(client, auth_tokens, paid_sale_creation):
    for role in ["admin", "manager", "cashier"]:
        # Create return
        resp = client.post(
            BASE_URL + "/returns",
            params={"sale_id": paid_sale_creation},
...
def test_start_return_unauthenticated(client, auth_tokens, paid_sale_creation):
    # Create return
    resp = client.post(
        BASE_URL + "/returns",
        params={"sale_id": paid_sale_creation},
...
def test_start_return_invalid_sale_id(client, auth_tokens):
    for role in ["admin", "manager", "cashier"]:
        # Create return
        resp = client.post(
            BASE_URL + "/returns",
...
def test_start_return_not_found(client, auth_tokens):
    for role in ["admin", "manager", "cashier"]:
        # Create return
        resp = client.post(
            BASE_URL + "/returns",
