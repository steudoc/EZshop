...
async def list_of_returns():
    async with await _get_session() as session:
        ...
        return2 = ReturnDAO(sale_id=sale1.id, status="CLOSED", closed_at=datetime.now())
        session.add(return2)
        await session.commit()
        ...
        return3 = ReturnDAO(sale_id=sale1.id, status="OPEN")
        session.add(return3)
        await session.commit()
...
@pytest.mark.asyncio
async def test_get_return_by_sale_id_empty(paid_sale):
    # Controller creation
    controller = ReturnController()

    result = await controller.get_returns_by_sale(sale_id=paid_sale.id)
    assert len(result) == 0

@pytest.mark.asyncio
async def test_get_returns_by_sale_success(list_of_returns):
    # Controller creation
    controller = ReturnController()

    result = await controller.get_returns_by_sale(sale_id=list_of_returns[0].sale_id)

    assert len(result) == 3
    assert all(isinstance(ret, ReturnDTO) for ret in result)
