...
@pytest.mark.asyncio
async def test_add_item_success_with_no_lines(instance_of_return):
    # Controller creation
    ...
    result = await controller.add_item(return_id=instance_of_return.id, item=item)

    assert isinstance(result, ReturnDTO)
    assert len(result.lines) == 1
    assert result.lines[0].product_barcode == "ABC123"
    assert result.lines[0].quantity == 2
    assert result.lines[0].price_per_unit == 10.0
    assert result.status == "OPEN"


@pytest.mark.asyncio
async def test_add_item_return_not_found():
    # Controller creation
    controller = ReturnController() 
    ...
    with pytest.raises(NotFoundError) as exc_info:
        result = await controller.add_item(return_id=99, item=item)

        assert result is None


@pytest.mark.asyncio
async def test_add_item_existing_line_increases_quantity(instance_of_return_with_existing_line):
    # Controller creation
    controller = ReturnController() 
    ...
    result = await controller.add_item(return_id=instance_of_return_with_existing_line.id, item=item)
