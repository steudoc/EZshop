...
@pytest_asyncio.fixture
async def instance_of_return_with_existing_line():
    async with await _get_session() as session:
        sale1 = SaleDAO(status="PAID")
        session.add(sale1)
        await session.commit()
...
@pytest.mark.asyncio
async def test_remove_item_return_not_found():
    # Controller creation
    ...
    with pytest.raises(NotFoundError) as exc_info:
        result = await controller.remove_item(return_id=99, product_barcode="ABC123", quantity=2)
...
@pytest.mark.asyncio
async def test_remove_item_success_delete(instance_of_return_with_existing_line):
    # Controller creation
    controller = ReturnController()

    result = await controller.remove_item(return_id=instance_of_return_with_existing_line.id, product_barcode="ABC123", quantity=2)
...
@pytest.mark.asyncio
async def test_remove_item_success_decrease(instance_of_return_with_existing_line):
    # Controller creation
    controller = ReturnController()

    result = await controller.remove_item(return_id=instance_of_return_with_existing_line.id, product_barcode="ABC123", quantity=1)

    assert isinstance(result, ReturnDTO)
    assert len(result.lines) == 1
    assert result.lines[0].quantity == 1
    assert result.lines[0].product_barcode == "ABC123"